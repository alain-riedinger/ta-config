...
-- Strings
lex:add_rule('quoted', lex:tag('quoted', lexer.range('"')))

-- Strikeout
lex:add_rule('strikeout', lex:tag('strikeout', P('~~') * (lexer.any - '~~')^0 * P('~~')^-1))

-- => conclusion
lex:add_rule('conclusion', lex:tag('conclusion', P('=>')))
-- -> action
lex:add_rule('action', lex:tag('action', P('->')))
-- /!\ warning
lex:add_rule('warning', lex:tag('warning', P('/!\\')))
-- (!) idea (?) question
lex:add_rule('idea', lex:tag('idea', P('(!)') + P('(?)')))
-- [ ] to do
lex:add_rule('todo', lex:tag('todo', P('[ ]')))
-- [x] done
lex:add_rule('done', lex:tag('done', P('[x]')))
lex:add_style('done', {fore = lexer.colors.teal, bold = true})

-- TEST
-- lex:add_rule('test', lex:tag('test', P('TEST')))
-- Put these 2 lines in theme lua file
-- lexer.colors.test = 0x164bcb
-- lex:add_style('test', {fore = colors.test, bold = true})
